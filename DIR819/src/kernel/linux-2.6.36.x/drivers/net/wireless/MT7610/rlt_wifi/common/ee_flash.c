...
//======================================================================//
/**
 * By sudenghai 2014-1-23
 *
 * TODO: Add TBS method to get wireless wifi Calibration Parameter
 * init func: tbs_ee_flash_init => get Calibration Parameter from tbs wifi item to eeprom struct
 * read func: tbs_ee_flash_offset_read/tbs_ee_flash_read_all => get some/all parameter from eeprom struct
 * write func: tbs_ee_flash_offset_write/tbs_ee_flash_write_all => write some/all parameter to eeprom struct
 * 
 */

static int  tbs_ee_flash_offset_read(IN PRTMP_ADAPTER pAd, USHORT offset, USHORT *value)
{
	int ret_val = -NDIS_STATUS_FAILURE;
	
	if(offset <= EEPROM_SIZE - sizeof(unsigned short)) {
		memcpy(value, pAd->chipCap.eebuf + offset, 2);
...
static void tbs_ee_flash_read_all(IN PRTMP_ADAPTER pAd, USHORT *data)
{	
	unsigned short len = EEPROM_SIZE;
	
	memcpy(data, pAd->chipCap.eebuf, len);
}


static void tbs_ee_flash_write_all(IN PRTMP_ADAPTER pAd, USHORT *data)
{
	size_t len = EEPROM_SIZE;
	int ret_val = -NDIS_STATUS_FAILURE;
	
	ret_val = item_save(pAd->chipCap.eebuf, WLAN_NAME_5G, len);
}
//====================================================================//
...
{	
	...
	else
	{
		...
		tbs_ee_flash_offset_read(pAd, Offset, pValue); //TODO: by T&W Sudenghai 2014-1-27
	}
	return (*pValue);
}
...
int rtmp_ee_flash_write(PRTMP_ADAPTER pAd, USHORT Offset, USHORT Data)
{
	if (init_flag)
	{
		...
		tbs_ee_flash_write_all(pAd, &Data); //TODO: by T&W Sudenghai 2014-1-27
	}
	return 0;
}
...
{	
	...
	tbs_ee_flash_read_all(pAd, Data); //TODO: by T&W Sudenghai 2014-1-27
}
...
static NDIS_STATUS rtmp_ee_flash_init(PRTMP_ADAPTER pAd, PUCHAR start)
{
        int len = EEPROM_SIZE;  /* e2prom size */

	init_flag = 1;          /* flag */
	nv_ee_start = start;    /* eebuf */

        if( item_get(start, WLAN_NAME_5G, &len) != 0 ){ /* Read item -> eebuf */
		...
		if (rtmp_ee_flash_reset(pAd, start) != NDIS_STATUS_SUCCESS)
		{
			...
			return NDIS_STATUS_FAILURE;
                ...
                //tbs_ee_flash_write_all(pAd, start); /* Write file -> item  */
                item_save(start, WLAN_NAME_5G, EEPROM_SIZE);/* Write file -> item  */
		...
		/* Random number for the last bytes of MAC address*/
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("The EEPROM in Flash is wrong, use default\n"));
                ...
                DBGPRINT(RT_DEBUG_ERROR, ("Flash read item ok !!!!!\n"));
