...
#define SIP_PROCFS_NAME "algonoff_sip"
#define MAX_PORTS	8
#if 0 //del by sudenghai 2014-2-25 for using tbs way
static unsigned char enable_sip = 0;    // sip ALG功能开启的标志，为0表示关闭，1表示开启
#endif
...
*/
static int proc_debug_read_sip(char *buffer, char **buffer_location, off_t offset, int buffer_length, int *eof, void *data)
{
    if (enable_sip == 1)
    {
        printk("sip ALG ON!\n");
...
*/
static int proc_debug_write_sip(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    if(proc_buffer[0] == '1')
    {
        printk("------Enable sip ALG function!------ [%s %s %d]\n", __FILE__, __FUNCTION__, __LINE__);
        enable_sip = 1;
    }
    if (proc_buffer[0] == '0')
    {
        printk("------Disable sip ALG function!------ [%s %s %d]\n", __FILE__, __FUNCTION__, __LINE__);
        enable_sip = 0;
    }
    return 1;
}
...
static int __init nf_conntrack_sip_init(void)
{
	...
	#if 0 //del by sudenghai 2014-2-25 for using tbs way
    struct proc_dir_entry* algonoff_sip;
    #endif

	if (ports_c == 0)
		ports[ports_c++] = SIP_PORT;

    if (ports_c == 1)       // 处理WAN侧主动发起拨号，WAN侧声音不能到LAN侧的问题
    {
        ports[ports_c++] = SIP_INVITE_PORT;
    }
	...
	#if 0 //del by sudenghai 2014-2-25 for using tbs way
    algonoff_sip = create_proc_entry(SIP_PROCFS_NAME,0644, NULL);
	if (algonoff_sip == NULL)
	{
		remove_proc_entry(SIP_PROCFS_NAME, NULL);
		printk(KERN_ALERT "Error: Could not initialize /proc/%s\n", SIP_PROCFS_NAME);
	}
    algonoff_sip->read_proc = (read_proc_t *)proc_debug_read_sip;
	algonoff_sip->write_proc = (write_proc_t *)proc_debug_write_sip;
	algonoff_sip->mode = S_IFREG | S_IRUGO;
    algonoff_sip->gid = 0;
    algonoff_sip->uid = 0;
    algonoff_sip->size = 4;
    #endif
