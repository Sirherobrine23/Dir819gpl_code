...
int ei_start_xmit(struct sk_buff* skb, struct net_device *dev, int gmac_no)
{
	...
	extern void dump_skb(struct sk_buff *skb, char *fun);
    if(dump_mask & DUMP_TX_UNTAG) {
        dump_skb(skb, "TX_TAGGED,ra_nat befor");
    }
#endif

    protocol = ntohs(skb->protocol);

    /*从通过vlanid转换成phyid,Added by pengyao 20131228*/
    /* lanport:eth2.1,eth2.2,eth2.3,eth2.4;wanport:eth2.5*/
    if(ETH_P_8021Q == protocol)
    {
        veth = (struct vlan_ethhdr *)(skb->data);
        iphy_id = (ntohs(veth->h_vlan_TCI) & VLAN_VID_MASK) - 1;
    }

#ifdef CONFIG_WAN_AT_P0 // switch port 0 as wan port
    if(0 == iphy_id)
#else   //CONFIG_WAN_AT_P4 for deault setting
    if(4 == iphy_id)
#endif
    {
