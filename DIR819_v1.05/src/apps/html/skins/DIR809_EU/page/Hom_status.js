...
`   <?if  et $11 1
	`	<?objget :InternetGatewayDevice.X_TWSZ-COM_IGMPProxy.IGMPTable. "GroupAddress Interface State"
		`	G_ProxyTables[m] = [];
			G_ProxyTables[m][0] = IntStr2IP("$01");
...
var Repeater_conStatus = [];
Repeater_conStatus['24conStatus'] = "<?get :InternetGatewayDevice.LANDevice.1.X_TWSZ-COM_ApCliConfiguration.1.ConnectRootAp.Connect?>";
Repeater_conStatus['5conStatus'] = "<?get :InternetGatewayDevice.LANDevice.1.X_TWSZ-COM_ApCliConfiguration.2.ConnectRootAp.Connect?>";
...
function initial_DSLITE() {
    (repeater_24G == "1" || repeater_5G == "1")?$("wan_ethernet_block").style.display = "none":$("wan_ethernet_block").style.display = "block";
	
	if (G_WANConn['Enable'] == "0" && G_TunConn['Activated'] == "1" && G_TunConn['Mode'] == "4in6")
	{	
...
function initial_WLAN(){
	if(repeater_24G == "1" || repeater_5G == "1"){
		$("gz_st_repeater_status").innerHTML = (Repeater_conStatus['24conStatus'] == "0"?"Disconnected":"Connected");
		$("gz_st_repeater_status_Aband").innerHTML = (Repeater_conStatus['5conStatus']== "0"?"Disconnected":"Connected");
	}
