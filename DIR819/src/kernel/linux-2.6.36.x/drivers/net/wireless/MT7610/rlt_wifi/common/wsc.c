...
#if 0		
			/* linsd add start disable wsc protocol in pin mode if AP do not support pin method */
			if( (WSC_PIN_MODE == pWscControl->WscMode) && (!(pWscControl->WscConfigMethods & 0xc)) )
			{
				rv = WSC_ERROR_WANTING_FIELD;
				goto Fail;
			}
			/* linsd end*/            
#endif             
...
#ifdef TW_PATCH_CONFIG_PERMIT
				if(pWscControl->WscConfStatus==WSC_SCSTATE_CONFIGURED)
  				{
				   DBGPRINT(RT_DEBUG_TRACE, ("WSC_SCSTATE_CONFIGURED\n"));
...
#ifdef WSC_V2_SUPPORT
		if (pWpsCtrl->WscV2Info.bEnableWpsV2)
			tempVal = pWpsCtrl->WscConfigMethods & 0xF9FF;/*0xF97F  for WPS PIN ON WIN7*/
		else
#endif /* WSC_V2_SUPPORT */
...
void sendSigToLogicRai0EnrollFin()
{
#ifdef LINUX

	/*
	     Notify user space application that WPS procedure finished.
	*/
	...
	OUT PWSC_PROFILE        pWscProfile)
{
    ...
    if((OpMode & ENROLLEE_ACTION))
   	{
        sendSigToLogicRai0EnrollFin();        
	}
...
/*
   add by TWSZ songyongbo 2016-03-31 start
   under unconfiged status,wps pin success,will change our ssid and security
   drv should notify logic to syn wireless config and save to config.xml
*/
#define SIG_RAI0_WPS_FIN (SIGRTMIN+5) /*37*/
void sendSigToLogicRai0()
{
#ifdef LINUX
...
#define WSC_LOGIC_APPNAME  "logic"
...
#endif
		{
				if(!strcmp(p->comm, WSC_LOGIC_APPNAME))
				{
					send_sig(SIG_RAI0_WPS_FIN, p, 0);
					printk("====>send_sig(SIG_RAI0_WPS_FIN)\n");
				}
		}
			read_unlock(&tasklist_lock);
	/* ---	added by YYHuang@Ralink, 08/03/12 */
...
/*
   add by TWSZ songyongbo 2016-03-31 end
   under unconfiged status,wps pin success,will change our ssid and security
   drv should notify logic to syn wireless config and save to config.xml
*/
...
out:
	...
	/*
	   ...
	   drv should notify logic to syn wireless config and save to config.xml
	*/	
		printk("WscWriteConfToDatFile()====>call sendSigToLogicRai0()\n");
		sendSigToLogicRai0();
	/*
	   add by TWSZ songyongbo 2016-03-31 end
	   under unconfiged status,wps pin success,will change our ssid and security
	...
	IN PVOID SystemSpecific3)
{
	...
	if (pWscControl->WscProfile.ApplyProfileIdx & 0x8000)
		bEnrollee = FALSE;
	/* add to send wsc status change message to userspace by netlink by rongwei on 2014/5/21 */
	DBGPRINT(RT_DEBUG_TRACE, ("Wps status has changed from unConfigured to Configured on 5.8G band.\n", __FUNCTION__));	
	DBGPRINT(RT_DEBUG_TRACE, ("Send status changed message from kernel to user-layer.\n", __FUNCTION__));
	tbs_netlink_send_msg( NL_CMD_WLAN_WSC_STATUS_CHG, "rai0" );
	/* end by rongwei on 2014/5/21 */
	WscWriteConfToPortCfg(pAd,
						  pWscControl,
						  &pWscControl->WscProfile.Profile[0],
						  bEnrollee);
