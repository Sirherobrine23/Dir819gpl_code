					   ...
					   struct in6_addr *mcastgrp)
{
	...
	struct in6_addr ZeroOrigin;
	memset(&ZeroOrigin, 0 , sizeof(struct in6_addr) );
...
/*
 * TBS_TAG: by pengyao 2012-08-29
 * Desc: 补充ip6mr_cache_find中对无源ip的多播组的通配查找
 */
    line = MFC6_HASH(mcastgrp, &ZeroOrigin);
    list_for_each_entry(c, &mrt->mfc6_cache_array[line], list) {
        if (ipv6_addr_equal(&c->mf6c_origin, &ZeroOrigin) &&
            ipv6_addr_equal(&c->mf6c_mcastgrp, mcastgrp))
            return c;
    }
/*
