...
#if defined(CONFIG_MTK_LINKSTATE)
...
//#define CONFIG_P5_RGMII_TO_MT7530_MODE 1
...
#define CONFIG_MTK_LINKSTATE
#if defined(CONFIG_MTK_LINKSTATE)
/*
  TBS-TAG: add by sudenghai at 2014-02-17
  wan link state detect for mon
*/
static int g_wan_status = 0;
static struct timer_list s_timer;
static void linkup_time_handle(unsigned long arg);
static int initPortStateCtrl(void);
...
#if 0
void dump_skb(struct sk_buff *sk, char *fun)
{
    unsigned int i;

    printk("%s: From %s with len %d (%d) headroom=%d tailroom=%d\n",
           fun, sk->dev?sk->dev->name:"ip stack",sk->len,sk->truesize,
...
void dump_skb(struct sk_buff *skb, char *fun)
{
    ...
    etdebug("%s: skb@%08x, data@%08x, len=%d, dev=%s\n", fun, (unsigned int)skb,
            (unsigned int)skb->data, skb->len, skb->dev ? skb->dev->name : "NULL");
    for(i = 0; i < skb->len; i++) {
        //      for(i = 0; i < 0x20; i++) {
...
#endif
static unsigned int ei_wait_addr_table(void)
{
    int i;
    unsigned int value;

    for(i = 0; ; i++) {
        value = reg_read32(GSW_ATC);
        if(0 == (value & BIT(15))) {/* Check whether address table is busy */
            break;
        } else if(i > 10000) {
            printk("%s: Table is busy!\n", __func__);
            break;
        } else {
            udelay(1);
        }
...
static void ei_write_addr_table_entry(int stat)
{
    unsigned int value;

    value = ei_wait_addr_table();
    value = (BIT(15) | BIT(0));/* Write 1 entry */
    value &= ~((0xF << 8) | (0x3 << 4));/* All MAC address entries */
    if(0 != stat) {/* Static entry */
        value |= (0x6 << 8);
    }
    reg_write32(GSW_ATC, value);
    ei_wait_addr_table();
}
...
#endif
{
    ...
    for ( ; ; ) {
        ...
        if(dump_mask & DUMP_RX_UNTAG) {
            dump_skb(rx_skb, "RX_TAGGED,ra_nat befor_1");
        }
#endif
...
#ifdef CONFIG_ETHERNET_DEBUG
        if(dump_mask & DUMP_RX_UNTAG) {
            dump_skb(rx_skb, "RX_TAGGED,ra_nat befor_2");
        }
#endif

/* lanport:eth2,eth3,eth4,eth5;wanport:eth1*/
/*Add by T&W Sudenghai 2014-3-15*/
/*没有通过具体的port id区分，是通过vlan做端口的区分，因为需要要考虑这种情况
  物理的port是固定的(0~4)，但是有的需求是更改LAN PORT的顺序和WAN PORT的位置，
  这时候只需要应用层更改vlan值，做到相应的的修改来区分，默认情况下WAN的vlan
  id 一直都是1，其他LAN的vlan id是2~5，在这里是分别减1，值为0~4*/
{
        protocol = ntohs(rx_skb->protocol);
        if(ETH_P_8021Q == protocol)
        {
            veth = (struct vlan_ethhdr *)(rx_skb->mac_header);
            ivlan_id = (ntohs(veth->h_vlan_TCI) & VLAN_VID_MASK);
#if defined(CONFIG_NETFILTER_XT_MATCH_PHYPORT)
			if(ivlan_id>=1 && ivlan_id<=5)
			{
	#ifdef CONFIG_WAN_AT_P4 
				/*实现设备名eth1~eht5 到端口4~0的转换*/
				rx_skb->srcPhyPort = ((5 - ivlan_id) > 0) ? (5 - ivlan_id) : (ivlan_id - 5);
                       	/*
                        * TBS_TAG: by Huangzheng 2014-10-16
                        * Desc:将下行流 nfmark 标记为0x80,上行流不标记默认
                        *          为0x0,以用于Qos区分上下行
                        */
                                 if(rx_skb->srcPhyPort==4)
                                       rx_skb->mark= (skb->mark & ~BASIC_NFMARK_DOWN) | (BASIC_NFMARK_DOWN & BASIC_NFMARK_DOWN);
                        /*TBS_END_TAG*/
	#else //CONFIG_WAN_AT_P0
				/*实现设备名eth1~eht5 到端口0~4的转换*/
				rx_skb->srcPhyPort = ivlan_id-1;
                       	/*
                        * TBS_TAG: by Huangzheng 2014-10-16
                        * Desc:将下行流 nfmark 标记为0x80,上行流不标记默认
                        *         为0x0,以用于Qos区分上下行
                        */
	                        if(rx_skb->srcPhyPort==0)
                                      rx_skb->mark= (skb->mark & ~BASIC_NFMARK_DOWN) | (BASIC_NFMARK_DOWN & BASIC_NFMARK_DOWN);
                        /*TBS_END_TAG*/
...
#ifdef CONFIG_WAN_AT_P4 
		/*wan port id is 4*/
		if(4 == rx_skb->srcPhyPort)
		{
#else //CONFIG_WAN_AT_P0
		/*wan port id is 0*/
		if(0 == rx_skb->srcPhyPort)
		{
#endif
			led_trigger_set(led_internet, led_blinking_trig);
		}
...
#if 0//defined (CONFIG_RT_3052_ESW) //delete by sudenghai 2014-2-17
/* modify by gxw 2013.3.12 for send signal USR2 to goahead */
...
static unsigned int linkUpTime[NR_PORTS] = {0,0,0,0,0};
...
static void linkup_time_handle(unsigned long arg)
{
	...
	{
		regData = reg_read32(RALINK_ETH_SW_BASE + 0x3008 + (port * 0x100));
		data0 = regData & IFF_UP;
		if (data0)
		{
			linkUpTime[port]++;
		}
...
static int port_state_read_proc( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	int	len;
	unsigned int regData;
	unsigned int data0;
	...
	for(port=0;port<NR_PORTS;port++)
	{
...
#ifdef   CONFIG_GE_RGMII_MT7530_P4_AN /* hefuhua add 20140813 */
                int sw_port = 0;      /*  temporary variable */

                if(port == 0)         /* hefuhua */
                        sw_port = 4;  /* port0 = port4 */
                else if(port == 4)
                        sw_port = 0;  /* port4 = port0 */
                else
                        sw_port = port; /*  */

		len += sprintf(page+len, "Port[%d]:", sw_port);
#else
		len += sprintf(page+len, "Port[%d]:", port);
#endif  /* CONFIG_GE_RGMII_MT7530_P4_AN */
		...
		len += sprintf(page+len, "RXPause:%s|", 1?"Enable":"Disable"); //temp
		len += sprintf(page+len, "TXPause:%s|", 1?"Enable":"Disable"); //temp

		data0 = (regData >> 1) & DUPLEX_FULL;/* 0: Half Duplex. 1: Full Duplex. */
		...
		len += sprintf(page+len, "Speed:%s|", data0==100?"100M":(data0==1000?"1000M":
				(data0==10?"10M":"Unkown")));
...
static int port_state_write_proc( struct file *filp, const char *buff,unsigned long len, void *data )
{
	return len;
}
...
static void esw_link_status_changed(int port_no, void *dev_id)
{
    ...
    */
	...
	if(0 == port_no)
#else //CONFIG_WAN_AT_P4
	if(4 == port_no)
#endif
	{
		vlan_dev = __find_vlan_dev(dev, 1); //vlan id 1 is always wan, and dev name is always eth1
		if(vlan_dev)
		{
			if ((reg_val & IFF_UP) != g_wan_status)
			{
			    printk("link changed, dev: %s, status: %d->%d\n",
			        vlan_dev->name, g_wan_status, reg_val & IFF_UP);
			    if (reg_val & IFF_UP)
			    {
			        netif_carrier_on(vlan_dev);
			        netif_start_queue(vlan_dev);
			    }
			    else
			    {
			        netif_stop_queue(vlan_dev);
			        netif_carrier_off(vlan_dev);
			    }
			    g_wan_status = reg_val & IFF_UP;
			}
		}
	}
#endif

//delete by sudenghai 2014-2-17
#if 0
...
void PHY_power_ops(unsigned int port, int optcode)
{
    unsigned int value = 0;
    if(port < 5) {
        mii_mgr_read(port, MII_BMCR, &value);
        if(optcode == ((value & BMCR_PDOWN) >> 11)) {/* Current state is not match witch optcode */
            if(0 == optcode) {
                value |= BMCR_PDOWN; /* Power down the PHY */
            } else {
#if 0
#ifdef CONFIG_WAN_AT_P0 // switch port 0 as wan port
                if(0 == port) {
#else// switch port 4 as wan port
                if(4 == port) {
#endif
                    value &= ~(BMCR_PDOWN | BMCR_ANENABLE);/* Disable NWAY for port 4 */
                    value |= (BMCR_FULLDPLX | BMCR_SPEED100);/* Force to 100Mbps, Full Duplex */
                } else
#endif
                {
                    value &= ~BMCR_PDOWN;
                    value |= (BMCR_ANENABLE | BMCR_ANRESTART); /* Restart auto-negotiation process */
                }
            }
...
void PHY_led_ops(unsigned int port, unsigned int optcode)
{
    ...
    //etdebug("Turn %s PHY LED on port %d\n", (led_off_trig == optcode) ? "OFF" : "ON", port);
...
#endif
#endif
    i = tbs_read_mac(LAN_MAC, 0, addr.sa_data);
...
#if defined (CONFIG_RT_3052_ESW)
	//INIT_WORK(&ei_local->kill_sig_wq, kill_sig_workq); //delete by sudenghai 2014-2-17
...
void ei_addr_table_init(void)
{
    int i;
    unsigned int value;

    for(i = 0; i <= NR_PORTS; i++) {
        reg_write32(GSW_PSC(i), 0xFFE20);
    }
    reg_write32(GSW_PSC(i), 0x10);/* Disable MAC learning on port 6 */
    value = ei_wait_addr_table();
    value = BIT(15) | BIT(1);/* Clean All entries */
    value &= ~((0xF << 8) | (0x3 << 4));/* All MAC address entries */
    reg_write32(GSW_ATC, value);
    ei_write_addr_table_entry(0);
    value = ei_wait_addr_table();
    value = (BIT(15) | BIT(2)); /* Start Search command (reset to 1st entry) */
    value &= ~((0xF << 8) | (0x3 << 4));/* All MAC address entries */
    reg_write32(GSW_ATC, value);
    ei_wait_addr_table();
}
...
/*Modify for TWSZ*/
	...
	*(unsigned long *)(RALINK_ETH_SW_BASE+0x3400) = 0x56330;//(P4, AN)
	//*(unsigned long *)(RALINK_ETH_SW_BASE+0x3400) = 0x8000;//(P4, LinkDown)
...
#ifdef CONFIG_GE_RGMII_MT7530_P4_AN /* */
	regValue &= ~((1<<5)|(1<<15));
	regValue |= ((1<<7)|(1<<16)|(1<<24));
	regValue |= ((1<<13)|(1<<6)); /*Disable Port5*/
#endif //CONFIG_GE_RGMII_MT7530_P4_AN
...
int __init ra2882eth_init(void)
{
    ...
    /*
     * TBS_TAG:add by pengyao 20130702
...
void fe_sw_init(void)
{
	...
	*(volatile u_long *)(SYSCFG1) &= ~(0x3 << 14);
    ...
    /*switch light leds*/
#ifndef CONFIG_APPS_LOGIC_INTERNET_LED_DLINK
    PHY_led_ops(0, led_blinking_trig);
#endif
}
...
EXPORT_SYMBOL_GPL(PHY_power_ops);
EXPORT_SYMBOL_GPL(ei_addr_table_init);
