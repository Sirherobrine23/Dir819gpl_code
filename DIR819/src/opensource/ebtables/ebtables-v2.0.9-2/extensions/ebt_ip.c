...
/* end merge */
	...
	case IP_DEST:
		...
		if (c == IP_SOURCE) {
			...
			parse_ip_range(optarg, ipinfo->saddr, ipinfo->smsk); // merge form MediaTek 2012-05-17
		}
		else {
			//ebt_parse_ip_address(optarg, &ipinfo->daddr, &ipinfo->dmsk);
			parse_ip_range(optarg, ipinfo->daddr, ipinfo->dmsk); // merge for MediaTek 2012-05-17
		}
		break;
...
**********************************************************/
	...
	if (ipinfo->bitmask & EBT_IP_PROTO) {
		struct protoent *pe;
#if 1  /* merge from MediaTek 2012-05-17 */
		struct protoent *pe2;
#endif
		printf("--ip-proto ");
