...
const ST_TBS_ERR_TABLE g_astTbsErrTable[] =
{
    ...
    {ERR_DHCPS_LEASE_TIME_OUT_RANGE, "The lease time is out of range","有效时间超出范围"},
...
const char* tbsGetMIDName(unsigned short usMID)
{
	...
	switch (usMID)
    {
		...
		MID_NAME(MID_GRP_LAN_DEVICE_DOWN);
		MID_NAME(MID_GRP_LAN_DEVICE_UP);
		MID_NAME(MID_GRP_IP6LAN_IFCON_CHANGED);
		MID_NAME(MID_GRP_IP6LAN_DNS_CHANGED);
		MID_NAME( MID_GRP_WAN_PROTOCOL_CHANGED);
		MID_NAME(MID_SYSLOG_SELF);
        MID_NAME(MID_NULL);
