					 ...
					 __be32 mcastgrp)
{
	...
	/*
	 * TBS_TAG: by yuanke 2011-3-3
	 * Desc: For IGMP Proxy (Merge from E8ponV3)
	 */
	origin = 0;
...
/*
 * TBS_TAG : by yuanke 2011-3-3
 * Desc : merge igmpv3 support from e8ponv3
 */
int ipmr_do_igmpv3_filter(struct sk_buff *skb)
{
    int rv = 1;
    struct in_device *in_dev;
    u32 mc_addr = ip_hdr(skb)->daddr;
    u32 src_addr = ip_hdr(skb)->saddr;
    ...
    if(rt->rt_flags & RTCF_MULTICAST)
    {
        ...
        if (ifindex) {
    		in_dev = inetdev_by_index(sock_net(skb->sk), ifindex);
    		if (in_dev)
    		{
                rv = ip_check_mc(in_dev, mc_addr, src_addr, IPPROTO_UDP);
                __in_dev_put(in_dev);
    		}
    	}
    }
			 ...
			 int local)
{
	...
	/* TBS_TAG : by yuanke 2011-3-3 Desc : merge igmpv3 support from e8ponv3 */
    if (!ipmr_do_igmpv3_filter(skb))
        goto dont_forward;
	/*
	 *	Forward the frame
	 */
...
int ip_mr_input(struct sk_buff *skb)
{
    ...
    /*Accept the upnp packet for emule software. 2012-9-20 dengyanbing */
    if(!local && ipv4_is_upnp(iph->daddr))
        local = 1;
