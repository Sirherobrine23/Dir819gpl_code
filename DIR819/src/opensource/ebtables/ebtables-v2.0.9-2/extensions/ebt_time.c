...
static unsigned int parse_minutes(const char *s)
{
	unsigned int hour, minute, second = 0;
	char *e;
    const char * pszTime = s;

	hour = strtoul(pszTime, &e, 10);
	if (*e != ':' || hour > 23)
		goto out;

	pszTime = e + 1;
	minute = strtoul(pszTime, &e, 10);
	if ((*e != ':' && *e != '\0') || minute > 59)
		goto out;
	if (*e == '\0')
		goto eval;
 ...
 out:
    ebt_print_error("invalid time %s specified, should be HH:MM:SS format", s);
	return -1;
}
      ...
      unsigned int *flags, struct ebt_entry_match **match)
{
	...
	case '1':
		...
		timeinfo->time_start = parse_minutes(optarg);
	...
	case '2':
		...
		timeinfo->time_stop = parse_minutes(optarg);
		*flags |= EBT_TIME_STOP;
		break;
...
static void
divide_time(int fulltime, int *hours, int *minutes, int *second)
{
	*hours = fulltime / 3600;
      ...
      const struct ebt_entry_match *match)
{
	...
	int hour_start, hour_stop, minute_start, minute_stop, s_start, s_stop;

	divide_time(time->time_start, &hour_start, &minute_start, &s_start);
	divide_time(time->time_stop, &hour_stop, &minute_stop, &s_stop);
	printf(" TIME from %02d:%02d:%02d to %02d:%02d:%02d on ",
	       hour_start, minute_start, s_start,
	       hour_stop, minute_stop, s_stop);
