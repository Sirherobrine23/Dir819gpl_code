...
/*defined DEV_NAME "eth"*/
#include ".././../../drivers/net/raeth/raether.h"
...
#if 0//For smarbit UDP small packet test. [lanql 2014-01-16]
...
/*PPTP LAN WAN need bond to CPU*/
	...
	return 1;
...
#endif
#endif
		/*TODO TBS, LAN vlan value is always 2/3/4/5, add by sudenghai 2014-3-5*/
		if ( ((foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) == 2) ||
	             ((foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) == 3) ||
	             ((foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) == 4) ||
	             ((foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) == 5) ) {
		...
		/*TODO TBS, WAN vlan value is always 1, add by sudenghai 2014-3-5*/
		else if ((foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) == 1) {
		    if ((bind_dir == UPSTREAM_ONLY) || (bind_dir == BIDIRECTION)) {
#if defined (CONFIG_RALINK_MT7621)
			PpeSetInfoBlk2(&foe_entry->ipv4_hnapt.iblk2, 1, 0x3F, 1);
...
#define MIN_NET_DEVICE_FOR_MESH                 0x30
	...
	else if (strncmp(skb->dev->name, DEV_NAME, strlen(DEV_NAME)) == 0) {
	    offset = DP_GMAC;
	}
...
static void PpeSetDstPort(uint32_t Ebl)
{
	...
	DstPort[DP_GMAC] = ra_dev_get_by_name(DEV_NAME);
#ifdef CONFIG_RAETH_GMAC2
...
#endif
#endif
		WanInt = ra_dev_get_by_name("eth1"); //TODO for TBS, eth1 is always wan dev
		
		if(!WanInt)
			printk(KERN_ERR "Unable to find WAN dev!\n");
...
{
    NAT_PRINT("Enable Ralink HW NAT...\n");
    ...
    if ((RegRead(0xB000000C) & 0xf) < 0x5) {
		...
		if(0 != SetAclFwd(1))
		{
			printk("HW NAT init fail!\n");
	
			PpeCleanupMod();
			return -1; // this API may fail!
		}
    } else {
