...

{
    ...
    if ((!algonoff_pptp)&&(alg_ip != iph->saddr)&&(alg_ip != iph->daddr))
    {
...
*/
static int proc_debug_read_pptp(char *buffer, char **buffer_location, off_t offset, int buffer_length, int *eof, void *data)
{
    if (algonoff_pptp == 1)
    {
        printk("pptp ALG ON!\n");
...
*/
static int proc_debug_write_pptp(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    if(proc_buffer[0] == '1')
    {
        printk("------Enable pptp ALG function!------ [%s %s %d]\n", __FILE__, __FUNCTION__, __LINE__);
        algonoff_pptp = 1;
    }
    else if (proc_buffer[0] == '0')
    {
        printk("------Disable pptp ALG function!------ [%s %s %d]\n", __FILE__, __FUNCTION__, __LINE__);
        algonoff_pptp = 0;
    }
    else
    {
...
user read something from kernel through proc file 
*/
static int proc_debug_read_algip(char *buffer, char **buffer_location, off_t offset, int buffer_length, int *eof, void *data)
{
    printk("lan ip: 0x%08x\n", alg_ip);
    return 0;
}
...
user write something to kernel through proc file
*/
static int proc_debug_write_algip(struct file *file, const char *buffer, unsigned long count, void *data)
{
    char proc_buffer[64];
	...
	if (!in4_pton(proc_buffer, strlen(proc_buffer), (void *)&addr, '\n', NULL))
	{
	    printk("bad ip address:[%s]\n", proc_buffer);
	}
	alg_ip = addr.ip;
    return count;
}
...
static int __init nf_conntrack_pptp_init(void)
{
	int rv;
	#if 0 //del by sudenghai 2014-2-25 for using tbs way
    struct proc_dir_entry* FP_Proc_algonoff_pptp;       // add by  dym
    struct proc_dir_entry* FP_Proc_alg_ip;   //add by gxw 2013.4.9
    #endif
	...
	#if 0 //del by sudenghai 2014-2-25 for using tbs way
// add by dym --add proc file 
    FP_Proc_algonoff_pptp = create_proc_entry(PROCFS_NAME,0644, NULL);
    if (FP_Proc_algonoff_pptp == NULL)
    {
        remove_proc_entry(PROCFS_NAME, NULL);
        printk(KERN_ALERT "Error: Could not initialize /proc/%s\n", PROCFS_NAME);
    }
    FP_Proc_algonoff_pptp->read_proc = (read_proc_t *)proc_debug_read_pptp;
    FP_Proc_algonoff_pptp->write_proc = (write_proc_t *)proc_debug_write_pptp;
    FP_Proc_algonoff_pptp->mode = S_IFREG | S_IRUGO;
    FP_Proc_algonoff_pptp->gid = 0;
    FP_Proc_algonoff_pptp->uid = 0;
    FP_Proc_algonoff_pptp->size = 4;
// end add by dym     

	//add by gxw 2013.4.9
    FP_Proc_alg_ip = create_proc_entry(PROCFS_ALGIP, 0644, NULL);
    if (FP_Proc_alg_ip == NULL)
    {
        remove_proc_entry(PROCFS_ALGIP, NULL);
        printk(KERN_ALERT "Error: Could not initialize /proc/%s\n", PROCFS_ALGIP);
    }
    FP_Proc_alg_ip->read_proc = (read_proc_t *)proc_debug_read_algip;
    FP_Proc_alg_ip->write_proc = (write_proc_t *)proc_debug_write_algip;
    FP_Proc_alg_ip->mode = S_IFREG | S_IRUGO;
    FP_Proc_alg_ip->gid = 0;
    FP_Proc_alg_ip->uid = 0;
    FP_Proc_alg_ip->size = 64;
	//end   
	#endif
...
static void __exit nf_conntrack_pptp_fini(void)
{
    ...
    remove_proc_entry(PROCFS_ALGIP, NULL);      // add by gxw 2013.4.9
    #endif
	nf_conntrack_helper_unregister(&pptp);
