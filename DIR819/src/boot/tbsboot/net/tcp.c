...
#if defined(CONFIG_AUTO_DETECT_DRAMSIZE)
extern unsigned long SYSCFG_OFFSET;
#endif
...
/*按照BT的要求去修改紧急升级页面的格式*/
#if defined(CONFIG_APPS_HTML_WEB_STYLE_BT) || defined(CONFIG_APPS_HTML_WEB_STYLE_BT_TEST) 
/*紧急升级页面*/
static	unsigned char indexdata[]="HTTP/1.0 200 OK\r\n\
Content-type: text/html\r\n\
\r\n\
<html>\
<head><title>Dual-Band Wi-Fi Extender - Recovery</title></head>\
<body>\
<form enctype=multipart/form-data method=post>\
<div style='text-align:center; color:blue;'><b>Dual-band Wi-Fi Extender Recovery Mode</b></div><br>\
<div style='text-align:center; height:80px;'>\
<span style='font-weight:bold'>Firmware File</span>\
<input type='file' name='firmware' size='15'/>\
<input type=submit value='Upload'>\
</div>\
</form>\
<hr />\
<div>\
<div><p>This page has been shown because the Hotspot firmware has been corrupted.The most likely reason is that a firmware update was interrupted.</p></div>\
<div><p>It will be necessary to upload new firmware onto the device.</p></div>\
<div><p>For instructions and latest device firmware please visit <b>bt.com/producthelp</b>. If you cannot find the information you require please call our free helpline on <b>0808 100 6116</b>.</p></div>\
<div style='color:red; font-weight:bold'><p>Using the wrong firmware file may result in the device not working properly or even to fail to start up.</p></div>\
</div>\
<hr />\
</body>\
</html>";
...
\r\n\
<html>\
<head><titleDual-Band Wi-Fi Extender - Recovery</title></head>\
<body>\
<form enctype=multipart/form-data method=post>\
<div style='text-align:center; height:80px;'>\
<span style='font-weight:bold'>Firmware File</span>\
...
<hr />\
<div style='text-align:center;'><p>Checksum validation failed! You should translate image again!</p></div>\
<hr />\
</body>\
</html>";
...
<hr />\
<div style='text-align:center;'><p>Update failure! Because your update image is too long ! Please select a small one.</p></div>\
<hr />\
</body>\
</html>";

/*选择的image文件格式错误时的报错页面*/
...
<hr />\
<div style='text-align:center;'><p>Update failure !Because your system is %s ,but you translate a %s image!</p></div>\
<hr />\
</body>\
</html>";
...
<div style='text-align:center; color:blue;'><b>Dual-band Wi-Fi Extender Recovery Mode</b></div><br>\
<hr />\
<div style='text-align:center;'><p>New software is being programmed to the flash memory. The Dual-Band Wi-Fi Extender will reboot upon completion. This process will take about 3 minutes.</p></div>\
<hr />\
</body>\
</html>";
#else
...
<br>\
NOTE: The update process takes about 3 minutes to complete, and your Wireless Router \
will reboot.<br>\
<br>\
<tr>\
...
<head><title>Update Firmware</title></head>\
<body>\
<b>Wireless Router Software Upgrade</b><br>\
New software is being programmed to the flash memory. The Wireless Router will reboot upon completion. This process will take about 3 minutes.\
</body> \
</html>";
...
unsigned short wfix(unsigned short a)
{
	unsigned short tmp;
	char * pchar = NULL;
	tmp = a;
	pchar = (unsigned char *)tmp;
	tmp = ( *((unsigned char *)&(pchar)) << 8 ) |( *( (unsigned char *)( (unsigned char *)&pchar + 1 ) ) );
	return tmp;
}
...
void tcp_Send(tcp_Socket *s)
{
    ...
    pkt->tcp.seqnum = lfix4(s->seqnum);
    pkt->tcp.acknum = lfix4(s->acknum);
    ...
    if ( s->flags & tcp_FlagSYN )
      ...
      pkt->maxsegopt = lfix4(0x02040578); /* 1400 bytes */
      dp += 4;
      }
...
int check_image( tcp_Socket *s,unsigned int len)
{
	...
	image_ok = 0;
      img_bin=0;
	sys_data = ( sys_config_t *  )SYSCFG_OFFSET;
	update_hdr = ( update_hdr_t * ) load_addr;
      image_bin_tail=(image_bin_tail_t *)( load_addr + len - 4 -4-PRODUCT_NAME_LEN);
...
/*modify by huangshuangbang 20110829 start*/
   // if( strcmp( PRODUCT, image_bin_tail->product) == 0&&strcmp( "bin", image_bin_tail->bin_type) == 0 )
    if( strcmp( "bin", image_bin_tail->bin_type) == 0 )
        {
        		/*modify by huangshuangbang 20110827 start*/
            	printf("\n\nUpgrade bin successfully.  File size=[%d]\n",len);          
		/*modify by huangshuangbang 20110827 end*/
...
/*del by huangshuangbang 20110829 start*/
#if 0 
...
#endif
/*del by huangshuangbang 20110829 end*/
...
void GotData(tcp_Socket *s, byte *dp, int len, int state)
{
	...
	if( ( state == 3 ) && ( image_ok == 1 ) )
	{
		syscfg =(sys_config_t*)SYSCFG_OFFSET;
            if(img_bin==0)
		    flash_update_img( ( unsigned char * ) load_addr,  syscfg , length, 1);
            else if(img_bin==1)
                 flash_update_bin(length, 0);
