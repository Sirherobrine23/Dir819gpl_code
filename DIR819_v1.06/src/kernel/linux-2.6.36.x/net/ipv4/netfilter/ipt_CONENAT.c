...
conenat_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
    ...
    mr = par->targinfo;
    newsrc = inet_select_addr(par->out, skb_rtable(skb)->rt_gateway, RT_SCOPE_UNIVERSE);
    if (!newsrc) {
	...
	if (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_UDP)
	{
		...
		if(expectcount > 0){
			for(tmpport=0; (tmpport<=expectcount)&&(newport<=65535); tmpport++,newport++){
				...
				exp = __nf_ct_expect_find_bysave(net, nf_ct_zone(ct), &tuple, &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
				if(exp)
					break;
			}
		}
