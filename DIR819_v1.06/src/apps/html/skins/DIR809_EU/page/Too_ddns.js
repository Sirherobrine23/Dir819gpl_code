...
function uiOnload(){
    ...
    setJSONValue({
			...
			"host"			: G_DDNS[1],
			"user"			: G_DDNS[2],
			"passwd"		: Base64.Decode(G_DDNS[4]),
			"passwd_verify"	: Base64.Decode(G_DDNS[4]),
			"timeout"		: G_DDNS[6],
			//"report"		: G_DDNS[3],
			"en_ddns_v6"	: G_DDNS[7],
			"v6addr"		: G_DDNS[9],
...
function uiSubmit()
{
	...
	$H({
		...
		":InternetGatewayDevice.X_TWSZ-COM_DDNS.1.Password"     : node_ddns[0].checked ? Base64.Encode($("passwd").value) : undefined,
