...
function createChannel_5G()
{
	...
	switch(Radio_list[1][7])
	{
		...
		case 'IN' :
		{
				new_options  = ['36','40','44','48','52','56','60','64','149','153','157','161','165'];
				new_values   = ['36','40','44','48','52','56','60','64','149','153','157','161','165'];
				break;
	    	}
		...
		case 'EU' :
		{
				//new_options = ['36','40','44','48','52','56','60','64','100','104','108','112','116','120','124','128','132','136','140'];
				//new_values  = ['36','40','44','48','52','56','60','64','100','104','108','112','116','120','124','128','132','136','140'];
...
function createChannel_5G_40M()
{
	...
	switch(Radio_list[1][7])
	{
		...
		case 'TW' :
		{
				new_options = ['149','153','157','161'];
				new_values  = ['149','153','157','161'];
				break;
	    	}
...
function repeaterModeDis(){
	for(var i=0; i<2; i+=1){
		var str_Aband=(i==0)?"":"_Aband";
		$('en_wifi'+str_Aband).setAttribute("disabled", true);
		$('sch'+str_Aband).setAttribute("disabled", true);
		$('go2sch'+str_Aband).setAttribute("disabled", true);
		$('wlan_mode'+str_Aband).setAttribute("disabled", true);
		$('auto_ch'+str_Aband).setAttribute("disabled", true);
		$('channel'+str_Aband).setAttribute("disabled", true);
		$('txrate'+str_Aband).setAttribute("disabled", true);
		$('bw'+str_Aband).setAttribute("disabled", true);
		$('ssid_visible'+str_Aband).setAttribute("disabled", true);
		$('ssid_invisible'+str_Aband).setAttribute("disabled", true);
	}
}
function routeModeEnable(){
	for(var i=0; i<2; i+=1){
		var str_Aband=(i==0)?"":"_Aband";
		$('en_wifi'+str_Aband).removeAttribute("disabled");
		$('sch'+str_Aband).removeAttribute("disabled");
		$('go2sch'+str_Aband).removeAttribute("disabled");
		$('wlan_mode'+str_Aband).removeAttribute("disabled");
		$('auto_ch'+str_Aband).removeAttribute("disabled");
		//$('channel'+str_Aband).removeAttribute("disabled");
		$('txrate'+str_Aband).removeAttribute("disabled");
		$('bw'+str_Aband).removeAttribute("disabled");
		$('ssid_visible'+str_Aband).removeAttribute("disabled");
		$('ssid_invisible'+str_Aband).removeAttribute("disabled");
	}
}
function uiOnload(){	
	createSchedule();
	...
	if(G_Mode == "WirelessRepeater") repeaterModeDis();
	else        routeModeEnable();
}
function initial(){
	for(var i=0; i<2; i+=1)
	{
		...
		if("40" == Radio_list[i][5])
		{
			$("bw"+str_Aband).value = "20O40";
		}
		else if("80" == Radio_list[i][5])
		{
		...
		//else
		{
			...
			if(G_Wireless[i][20].length==0)
				$("radius_srv_sec"+str_Aband).value = G_Wireless[i][20];
			else
...
function uiSubmit()
{
	...
	case 'wep' : {
		...
		{
			if(!$("wep_128_1").value.match(/^[0-9a-zA-Z\\.@_$-]{1,26}$/))
			{
				alert(SEcode["lang_invalid_passwd"]);
				return false;
			}
	...
	case 'wep' : {
		...
		{
			if(!$("wep_128_1_Aband").value.match(/^[0-9a-zA-Z\\.@_$-]{1,26}$/))
			{
				alert(SEcode["lang_invalid_passwd"]);
				return false;
			}
