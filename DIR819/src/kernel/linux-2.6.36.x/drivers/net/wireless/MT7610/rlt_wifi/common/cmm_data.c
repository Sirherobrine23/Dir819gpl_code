...
#ifdef GUEST_ACCESS_WEB_FILTER
#define WEB_SERVER_PROTOCOL 	0x06 	/* tcp */
#define WEB_SERVER_PORT 		80		/* http port */
#define SAMBA_SERVER_PORT       139         //samba
#define ICMP_PROTOCOL 	0x01 	/* icmp linsd add  */

/**********************************************************************
 * Desc: pick up and drop thess packages which sent to local bridge device
 *       and try to access manage page on 80 port using http protocol
 *       from gueste network
 * @pAd: wireless device struct pointer
 * @pIpHeader: point ip head
 *
 * @return: 0(legal packs), 1(illegal packs)
 *********************************************************************/
static int guest_access_web_filter( PRTMP_ADAPTER pAd, UCHAR *pIpHeader )
{
	unsigned int ipHdrLen = 0;				/* ip header length */
	unsigned int protocol = 0;				/* protocol type, such as tcp */
	unsigned int dstIp = 0;					/* destination ip address */
	unsigned short dstPort = 0; 			/* destination port number */
	struct net_device * pBridgDev = NULL;	/* bridge device pointer */
	struct in_ifaddr *ifap = NULL;
	struct in_device *in_dev = NULL;

	/* 1.check protocol type */
	protocol = *( pIpHeader + 9 );
	if( (WEB_SERVER_PROTOCOL != protocol) && (ICMP_PROTOCOL != protocol)  )
	{
		return 0;	// return immediately if it's non tcp protocol
	}

	if (WEB_SERVER_PROTOCOL == protocol)
	{
		/* 2.check dest port */
		ipHdrLen = ( (*pIpHeader) & 0x0f ) << 2; /* ip header length */
		dstPort = *(unsigned short *)( pIpHeader + ipHdrLen + 2 );	/* dest port number */
		if(( WEB_SERVER_PORT != ntohs(dstPort) )&&( SAMBA_SERVER_PORT != ntohs(dstPort) ))	
		{
			return 0;	// return immediately if the port number is not 80
		}	

	}

	/* 3.check dest ip address */
	pBridgDev = RtmpOSNetDevGetByName( pAd->net_dev, "br0" );
	in_dev = (struct net_device *)( pBridgDev->ip_ptr );
	if( in_dev )
	{
		dstIp = *(unsigned int *)( pIpHeader + 16 );
		for( ifap = in_dev->ifa_list; ifap != NULL; ifap = ifap->ifa_next )
		{
			if( 0 == strcmp( pBridgDev->name, ifap->ifa_label ) )
			{
				if( ifap->ifa_address != dstIp )
				{
					return 0;	// return immediately if the dest address is not the bridge address
				}
			}
		}
	}
...
//---Add by shiang for debug
	...
	*/
	...
	#ifdef GUEST_ACCESS_WEB_FILTER
	/* add to stop guest network to access manage page by rongwei on 2014/06/11 */
	if((pAd->ApCfg.IsolateInterStaTrafficBTNBSSID == 1) && ( FromWhichBSSID > 0 ) && ( FromWhichBSSID < MIN_NET_DEVICE_FOR_WDS ) ) /* skip packages from non-guest network */	
	{
		if( guest_access_web_filter( pAd, pRxBlk->pData ) )
		{
			/* release packet */
	...
	/* end by rongwei on 2014/06/11 */
	#endif  /* GUEST_ACCESS_WEB_FILTER */
