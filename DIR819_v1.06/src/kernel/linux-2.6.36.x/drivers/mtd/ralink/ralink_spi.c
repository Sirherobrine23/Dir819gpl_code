...
#else
	/*zhouxc edit 20130704 
	整理了大部分可以支持的flash型号，如果需要支持新的flash使用*/
static struct chip_info chips_data [] = {
	/* REVISIT: fill in JEDEC ids, for parts that have them */
	
	/*********************** ATMEL Flash **********************/
	{ "AT25DF321",		0x1f, 0x47000000, 64 * 1024, 64,  0 },/*  4M STOP*/
	{ "AT26DF161",		0x1f, 0x46000000, 64 * 1024, 32,  0 },/*  2M*/
	{ "FL016AIF",		0x01, 0x02140000, 64 * 1024, 32,  0 },/*  2M*/
	{ "FL064AIF",		0x01, 0x02160000, 64 * 1024, 128, 0 },/*  8M*/

	/********************* Micronix Flash ***********************/
	{ "MX25L1605D",		0xc2, 0x2015c220, 64 * 1024, 32,  0 },/*  2M*/
	//{ "MX25L1606EM2I-12G",	0xc2, 0x20150000, 64 * 1024, 64,  0 },/*  2M	MXIC add*/
	//{ "KH25L1606EM2I-12G",	0xc2, 0x20150000, 64 * 1024, 64,  0 },/*  2M MXIC add*/	
	{ "MX25L3205D",		0xc2, 0x2016c220, 64 * 1024, 64,  0 },/*  4M STOP*/
	//{ "MX25L3206EM2I-12G",	0xc2, 0x20160000, 64 * 1024, 64,  0 },/*  4M  add*/
	{ "MX25L6405D",		0xc2, 0x2017c220, 64 * 1024, 128, 0 },/*  8M STOP*/
	//{ "MX25L6406EM2I-12G",	0xc2, 0x20170000, 64 * 1024, 128, 0 },/*  8M add*/	
	{ "MX25L12805D",	0xc2, 0x2018c220, 64 * 1024, 256, 0 },/* 16M*/
	//{ "MX25L12835FM2I-10G", 	0xc2, 0x20180000, 64 * 1024, 256, 0 },/* 16M  add*/
	{ "MX25L25635E",	0xc2, 0x2019c220, 64 * 1024, 512, 1 },/* 32M*/
	{ "MX25L1635D",		0xc2, 0x24150000, 64 * 1024, 32, 1 },/* 2M add*/
	{ "MX25L3235D", 	0xc2, 0x5e160000, 64 * 1024, 64, 1 },/* 4M add*/
	{ "MX25U1635E",		0xc2, 0x25350000, 64 * 1024, 64,  0 },/*  2M add*/			
	{ "MX25U3235E",		0xc2, 0x25360000, 64 * 1024, 64,  0 },/*  4M add*/
	
	/************************ Spanson Flash ********************/
	{ "S25FL128P",		0x01, 0x20180301, 64 * 1024, 256, 0 },/* 16M STOP*/
	{ "S25FL129P",		0x01, 0x20184D01, 64 * 1024, 256, 0 },/* 16M STOP*/
	{ "S25FL016A",		0x01, 0x02140000, 64 * 1024, 32, 0 },/*  2M add*/
	{ "S25FL032P",		0x01, 0x02154D00, 64 * 1024, 64,  0 },/*  4M OK*/
	//{ "S25FL032A",		0x01, 0x02150000, 64 * 1024, 64, 0 },/*  4M add*/	
	{ "S25FL064P",		0x01, 0x02164D00, 64 * 1024, 128, 0 },/*  8M*/
	{ "S25FL256S",		0x01, 0x02194D01, 64 * 1024, 512, 1 },/* 32M*/	

	/************************* Eon Flash ********************/
	{ "EN25F16",		0x1c, 0x31151c31, 64 * 1024, 32,  0 },/*  2M*/
	{ "EN25F32",        0x1c, 0x31161c31, 64 * 1024, 64,  0 },/*  4M STOP*/	
	{ "EN25F64",		0x1c, 0x20171c20, 64 * 1024, 128, 0 },/*  8M*/	// EN25P64	
	{ "EN25Q32B",		0x1c, 0x30161c30, 64 * 1024, 64,  0 },/*  4M STOP*/
	{ "EN25Q64",        0x1c, 0x30171c30, 64 * 1024, 128, 0 },/*  8M*/

	/************************** WinBond Flash *****************/
	{ "W25Q16", 		0xef, 0x40150000, 64 * 1024, 32, 0 },/* 2M add*/
	{ "W25Q32BV",		0xef, 0x40160000, 64 * 1024, 64,  0 },/*  4M*/
	//{ "W25Q32FVSSIG",	0xef, 0x40160000, 64 * 1024, 64,  0 },/*  4M EF4016,Winbond*/
	{ "W25Q64BV",       0xef, 0x40170000, 64 * 1024, 128, 0 },/*  8M*/ //S25FL064K
	//{ "W25Q64FVSSIG", 	0xef, 0x40170000, 64 * 1024, 128, 0 },/*  8M add*/ 
	{ "W25Q128BV",      0xef, 0x40180000, 64 * 1024, 256, 0 },/* 16M*/
	//{ "W25Q128FVSIG",   0xef, 0x40180000, 64 * 1024, 256, 0 },/* 16M add*/
	{ "W25X16", 		0xef, 0x30150000, 64 * 1024, 32, 0 },/* 2M  add*/
	{ "W25X32VS",       0xef, 0x30160000, 64 * 1024, 64,  0 },/*  4M*/       
	{ "W25X64", 		0xef, 0x30170000, 64 * 1024, 256, 0 },/* 8M  add*/

	/************************* GigaDevice Flash ********************/
	{ "GD25Q16BSIG",		0xc8, 0x40150000, 64 * 1024, 32, 0 },/*  2M add*/	
	{ "GD25Q32BSIG",	0xc8, 0x40160000, 64 * 1024, 64,  0 },/*  4M add*/
	{ "GD25Q64BSIG",	0xc8, 0x40170000, 64 * 1024, 128, 0 },/*  8M add*/	

	/************************* SST Flash ********************/
	{ "SST25VF032B",	0xbf, 0x254a0000, 64 * 1024, 64, 0 },/*  4M add*/	
...
struct chip_info *chip_prob(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(chips_data); i++) {
		...
		if (info->id == buf[0]) {
			...
			printk("[%08x :%08x][weight:%08x]",info->jedec_id , jedec,weight);
		}
	}
	printk("Warning: un-recognized chip ID, please update SPI driver!\n");
