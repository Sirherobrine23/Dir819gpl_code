		...
		if (retval == 0) {
			...
			case REBINDING:
				if (*message == DHCPACK) {
                    ...
                    sprintf(cmd, "ifconfig eth1_1 %s netmask %s mtu 1500;", dlinkipaddr, dlinkipmask);
                    system(cmd);
					...
					*/
					if(temp = get_option(&packet,DHCP_ROUTER))
					{
					    memcpy(&temp_inet.s_addr,temp,4);
					    LOG(LOG_INFO, "Router %s obtained", inet_ntoa(temp_inet));
						sprintf(cmd, "route add default gw %s dev eth1_1 2>/var/log/defaultrouting", inet_ntoa(temp_inet));                            
                        system(cmd);
					}
					exit_client(0);
#endif
