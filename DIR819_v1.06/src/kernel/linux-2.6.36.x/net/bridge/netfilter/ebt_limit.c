...
/*
 * TBS_TAG: add by cz 2012/11/6
 * Description:  one second = 1000 millsecond 
 *              
*/ 
#define MILLISEC_PER_SEC    1000
#define MAX_CPJ (0xFFFFFFFF / (MILLISEC_PER_SEC*60*60*24))
...
user2credits(u_int32_t user)
{
	...
	if (user > 0xFFFFFFFF / (MILLISEC_PER_SEC*CREDITS_PER_JIFFY))
		/* Divide first. */
		return (user / EBT_LIMIT_SCALE) * MILLISEC_PER_SEC * CREDITS_PER_JIFFY;

	return (user * MILLISEC_PER_SEC * CREDITS_PER_JIFFY) / EBT_LIMIT_SCALE;
}
