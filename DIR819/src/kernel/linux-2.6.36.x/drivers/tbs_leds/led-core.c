...
led_name multiple_use_led = led_wlan;	/* multi-use led name, such as led_wlan */
#endif
...
int led_trigger_set(led_name led, trig_name trigger)
{
	...
	//gxw / 2014-10-16 / 对于关闭的LED灯禁止led_blinking_trig动作
	//解决internet灯关闭后，被led_blinking_trig动作点亮问题
	if(p_led->cur_trig) {
		if (led_off_trig == p_led->cur_trig->name && led_blinking_trig == trigger)
			return 0;
	}
