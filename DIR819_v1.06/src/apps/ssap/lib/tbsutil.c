...
int tbsMacToStr(char *pszSrcMac, char *pszDstMac)
{
	int i = 0;

	if(NULL == pszSrcMac || NULL == pszDstMac || strlen(pszSrcMac) > MAX_MAC_ADDR_LEN)
	{
		return TBS_FAILED;
	}
	
	for(i = 0; i < strlen(pszSrcMac); i++)
	{
		if(":" != *(pszSrcMac+i))
		{
			*(++pszDstMac) = *(pszSrcMac+i);
		}
	}
