...
{	
	...
	$("scanUplink5G").disabled = true;
	$("scanUplink5G_tip").style.display = "block";
	$("SEL_SCAN").style.display = "none";
	// set flag to '2', and then submit
	timerStart();
	var tempPath = G_arrClient[1][0]+'ApCliTriggerScan';
...
function crtNetWorkList(){
	...
	for(instanceIdx; instanceIdx<s_len5; instanceIdx++)
	{
		...
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][0]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][1]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][2]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][3]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][4]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][5]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][6]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][7]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][8]);
		S_arrTemp[instanceIdx].push(G_arrScan5[instanceIdx][9]);
		instanceTotal++;			
	}
...
{ 
	if (wtime >= 0)
	{ 
		$("scanUplink5G_time").innerText = wtime;
		//$("scanUplink5G").value = "Surveying, please don't interrupt, remaining "+wtime+" s";
		wtime--;
		setTimeout("timerStart()",1000);
	}
	else
	{
		$("scanUplink5G_tip").style.display = "none";
		top.document.location.href="/cgi-bin/webproc?getpage=html/index.html&var:menu=basic&var:page=5G_repeater&var:Startscan=1";
	}
}
