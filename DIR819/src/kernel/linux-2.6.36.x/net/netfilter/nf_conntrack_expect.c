		    ...
		    const struct nf_conntrack_tuple *tuple)
{
	...
	hlist_for_each_entry(i, n, &net->ct.expect_hash[h], hnode) { //Modify by Sudenghai for corenat
...
/*
 * TBS_TAG: by Sudenghai 2014-2-24
 * Desc:Add for conenat
 */
static inline int
...
struct nf_conntrack_expect *
__nf_ct_expect_find_bysave(struct net *net, u16 zone,
			const struct nf_conntrack_tuple *tupleMake, const struct nf_conntrack_tuple *tuple)
{
	struct nf_conntrack_expect *i;
	struct hlist_node *n;
	unsigned int h;
	...
	hlist_for_each_entry(i, n, &net->ct.expect_hash[h], hnode) {
		...
		if (exp_src_cmp(i, tuple) && nf_ct_zone(i->master) == zone){
				return i;
			}
	}
	return NULL;
