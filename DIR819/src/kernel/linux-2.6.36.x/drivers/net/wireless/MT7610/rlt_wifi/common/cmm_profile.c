					...
					if (RTMPGetKeyParameter(tok_str, tmpbuf, 128, buffer, FALSE))
						{
							...
							else
							{
							    ...
							    for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
							    {
									//如果主网络没有用WEP加密，访客网络用WEP加密时，会导致访客网络
									//无法连接上，这是因为格式为:Key*Str=;******的形式
									if(tmpbuf[0]==';')
									{ 
										rtmp_parse_key_buffer_from_file(pAd, macptr, KeyType[i+1], i+1, idx);
									}
									else
...
/*	tmpbuf = kmalloc(MAX_PARAM_BUFFER_SIZE, MEM_ALLOC_FLAG);*/
	...
	/*#endif*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			/*AutoChannelSkipList*/
			if (RTMPGetKeyParameter("AutoChannelSkipList", tmpbuf, 64, pBuffer, FALSE))/* hefuhua mod 20140823 */
			{		
				pAd->ApCfg.AutoChannelSkipListNum = delimitcnt(tmpbuf, ";") + 1;
				if ( pAd->ApCfg.AutoChannelSkipListNum > MT7610_ACSL_NUM ) /* hefuhua mod 20140823 */
				{
					DBGPRINT(RT_DEBUG_TRACE, ("Your no. of AutoChannelSkipList( %d ) is larger than 10 (boundary)\n",pAd->ApCfg.AutoChannelSkipListNum));
					pAd->ApCfg.AutoChannelSkipListNum = MT7610_ACSL_NUM;/* hefuhua mod 20140823 */
				}
