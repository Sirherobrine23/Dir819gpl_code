...
#if 0 //del by sudenghai 2014-2-25 for using tbs way
static unsigned char algonoff_ipsec= 0;
#endif
		      ...
		      unsigned int hooknum)
{
	...
	if (ct->status & IPS_SEEN_REPLY) {
		...
		nf_conntrack_event_cache(IPS_SRC_NAT, ct);
...
*/
static int proc_debug_read_ipsec(char *buffer, char **buffer_location, off_t offset, int buffer_length, int *eof, void *data)
{
    if (algonoff_ipsec == 1)
    {
        printk("ipsec ALG ON!\n");
...
*/
static int proc_debug_write_ipsec(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    if(proc_buffer[0] == '1')
    {
        printk("------Enable ipsec ALG function!------ [%s %s %d]\n", __FILE__, __FUNCTION__, __LINE__);
        algonoff_ipsec = 1;
    }
    else if (proc_buffer[0] == '0')
    {
        printk("------Disable ipsec ALG function!------ [%s %s %d]\n", __FILE__, __FUNCTION__, __LINE__);
        algonoff_ipsec = 0;
    }
    else
    {
...
static int __init nf_conntrack_ipsec_init(void)
{
    ...
    char *tmpname;
    #if 0 //del by sudenghai 2014-2-25 for using tbs way
	struct proc_dir_entry* FP_Proc_algonoff_ipsec;
	#endif
	...
	#if 0 //del by sudenghai 2014-2-25 for using tbs way
	FP_Proc_algonoff_ipsec = create_proc_entry(PROCFS_NAME,0644, NULL);
	if (FP_Proc_algonoff_ipsec == NULL)
	{
		remove_proc_entry(PROCFS_NAME, NULL);
		printk(KERN_ALERT "Error: Could not initialize /proc/%s\n", PROCFS_NAME);
	}
	FP_Proc_algonoff_ipsec->read_proc = (read_proc_t *)proc_debug_read_ipsec;
	FP_Proc_algonoff_ipsec->write_proc = (write_proc_t *)proc_debug_write_ipsec;
	FP_Proc_algonoff_ipsec->mode = S_IFREG | S_IRUGO;
    FP_Proc_algonoff_ipsec->gid = 0;
    FP_Proc_algonoff_ipsec->uid = 0;
    FP_Proc_algonoff_ipsec->size = 4;
    #endif 
